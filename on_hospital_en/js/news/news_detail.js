$(document).ready(function(){
    ...
    function fn_article3(containerID, buttonID, autoStart){
        var $element = $('.mr_main_comment').find('p');
        var $prev = $('.news_de_prev');
        var $next = $('.news_de_next');
        var auto = null;
        var speed = 2000;
        var timer = null;
    
        var move = $element.children().outerHeight();//.notice-list ol li의 높이값
        var first = false;
        var lastChild;
    
        lastChild = $element.children().eq(-1).clone(true);//li의 마지막 요소를 복사합니다.
        lastChild.prependTo($element);//복사한 것을 앞에 추가합니다.
        ...
        $prev.bind({
            'click': function(){ //이전 버튼을 눌렀을 경우, 
                movePrevSlide(); //슬라이드 실행
                return false;
            },
            'mouseenter': function(){ //이전 버튼에 마우스를 올렸을 경우 timer함수를 제거. 부딪침을 방지하기 위함
                if(auto){
                    clearInterval(timer);
                }
            },
            'mouseleave': function(){//이전 버튼에서 마우스를 뗐을때는 다시 함수를 실행.
                if(auto){
                    timer = setInterval(moveNextSlide, speed);
                }
            }
        ...
        function movePrevSlide(){ //movePrevSlide함수!! 
            $element.each(function(idx){
                if(!first){//first는 처음 선언한 false가 담긴 변수.
